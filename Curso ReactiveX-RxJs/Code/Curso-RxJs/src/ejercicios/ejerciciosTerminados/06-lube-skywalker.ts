...
import { switchMap, map, mergeAll, pluck, concatMap } from 'rxjs/operators';
...
(() =>{
    ...
    const humano$ = getRequest(`${ SW_API }/people/1/`);
    const especie$ = humano$.pipe(
        pluck('species'), 
        concatMap(getRequest)
    )

    zip(humano$, especie$).pipe(
