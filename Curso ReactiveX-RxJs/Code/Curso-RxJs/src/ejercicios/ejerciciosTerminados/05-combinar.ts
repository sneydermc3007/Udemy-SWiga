...
(() =>{
    ...
    //letras$.combineLatest(numeros$).subscribe(next => console.log(next))

        //Mi solucion
    // const combinacion$ = combineLatest(letras$, numeros$)
    //     .subscribe((value) => console.log(value));
    
    // Solucion del profesor
    const combinacion$ = combineLatest(letras$, numeros$).pipe(
        map(([a,b]) => a + b)
    )
    .subscribe((value) => console.log(value));    
})();
